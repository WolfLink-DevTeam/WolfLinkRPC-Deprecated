...
public class EnableServerPlugin extends SimpleCommandAnalyse {
    ...
    public IAction getAction() {
        return datapack -> {
            ...
            if(plugin.isEnabled())
            {
                MQService.INSTANCE.sendCommandFeedBack(datapack,new SimpleCommandResultBody(false,"插件已经被启用了，请勿重复操作"));
                return;
            }
            Bukkit.getPluginManager().enablePlugin(plugin);
            MQService.INSTANCE.sendCommandFeedBack(datapack,new SimpleCommandResultBody(false,"插件启用成功"));
        };
    }
}
